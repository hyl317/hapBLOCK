...
def hapBLOCK_chrom(folder_in="./data/hdf5/1240k_v43/ch", iids = ["", ""], 
                   ...
                   cutoff_post=0.99, max_gap=0.0075, snp_gap=0.005, save=0):
    ...
    h.p_obj.set_params(ch=ch, min_cm1=min_cm1, min_cm2=min_cm2, cutoff_post=cutoff_post, max_gap=max_gap, snp_gap=snp_gap, folder=folder_out, save=save)
...
def hapBLOCK_pair(folder_in="./data/hdf5/1240k_v43/ch", iids = ["", ""], 
    ...
    dfs = []
    for ch in chs:
        df, *_ = hapBLOCK_chrom(folder_in, iids, ch, folder_out, output, prefix_out, logfile, l_model, IBD2, p_col, ibd_in, ibd_out, ibd_jump, min_cm1, min_cm2, cutoff_post, max_gap, snp_gap, save)
        dfs.append(df)
...
def hapBLOCK_all(folder_in=None, iids=[], chs=range(1,23), folder_out="", output=False, prefix_out="", logfile=False,
                    ...
                    cutoff_post=0.99, max_gap=0.0075, snp_gap=0.005, save=3, nprocesses=4):
    """
    ...
    prms = [ [folder_in, [id1, id2], 
                   ...
                   cutoff_post, max_gap, snp_gap, save] for id1, id2 in it.combinations(iids_dedup, 2)]
